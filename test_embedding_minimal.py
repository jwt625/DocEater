...
DATABASE_URL = os.getenv("DOCEATER_DATABASE_URL", "postgresql://localhost:5432/doceater")
...
class MinimalEmbeddingTest:
    ...
    async def test_image_embeddings(self):
        ...
        # Store in database
        for desc, embedding in zip(descriptions, embeddings):
            await self.conn.execute("""
                INSERT INTO test_embeddings (content_type, content, embedding)
                VALUES ($1, $2, $3)
            """, "image", desc, embedding.tolist())
    ...
    async def test_similarity_search(self):
        ...
        for i, row in enumerate(results, 1):
            print(f"  {i}. [{row['content_type']}] {row['content'][:60]}... "
    ...
    async def test_cross_modal_search(self):
        ...
        for i, row in enumerate(results, 1):
            print(f"  {i}. {row['content']} (similarity: {row['similarity_score']:.3f})")
