...
class TestSearchEndpoints:
    ...
    def test_search_documents_not_implemented(
        ...
        response = test_client.post(
            "/api/v1/search", json=search_request, headers=auth_headers
        )
    ...
    def test_search_similar_documents_not_implemented(
        ...
        response = test_client.post(
            "/api/v1/search/similar", json=similar_request, headers=auth_headers
        )
    ...
    def test_search_documents_invalid_request(
        ...
        invalid_request = {"top_k": 10}

        response = test_client.post(
            "/api/v1/search", json=invalid_request, headers=auth_headers
        )
    ...
    def test_search_similar_documents_invalid_request(
        ...
        response = test_client.post(
            "/api/v1/search/similar", json=invalid_request, headers=auth_headers
        )
